...
= Floquetifying Quantum Error Correction Code into Process Code

== Process Code

A process code is defined by a set of operations $cal(O) = [O_1, O_2, ...]$ where $O_i$ can be either Pauli string measurement or Clifford gates.

The process code is said to be *established* after $T$ rounds of operations. Established means the size of *Instantaneous Stabilizer Group* (ISG) @hastings2021dynamically is constant for $t > T$. For $t > T$, the process code always encodes a constant number of logical qubits @rodatz2024floquetifying. 

Examples of process code include stabilizer codes, subsystem codes, and Floquet codes.
